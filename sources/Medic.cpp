...
namespace pandemic{
    ...
    Player& Medic::treat(City _city){
        if (city != _city) {
        throw invalid_argument("The city " + city_by_string.at(city) + " not " + city_by_string.at(_city));
        }
        if (board.disease_level[_city] == 0) {
            throw invalid_argument("There is no a cards at " + city_by_string.at(_city));
        }
        board.disease_level[_city] = 0;
        return *this;
    }
    
    // when he drive to any city the medic cure.
    ...
    // when he fly direct to any city medic cure.
